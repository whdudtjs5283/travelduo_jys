...
public class Product_Hoogi_Service {
	...
	public ArrayList<Product_Hoogi> hoogiSearchTitle(String word, int currentPage, int limit) {
		Connection conn = getConnection();
		ArrayList<Product_Hoogi> list = Pdao.hoogiSearchTitle(conn, word, currentPage, limit);
		close(conn);
		return list;
	}

	public ArrayList<Product_Hoogi> hoogiSearchContent(String word, int currentPage, int limit) {
		Connection conn = getConnection();
		ArrayList<Product_Hoogi> list = Pdao.hoogiSearchContent(conn, word, currentPage, limit);
		close(conn);
		return list;
	...
	public ArrayList<Product_Hoogi> hoogiSearchTCmix(String word, int currentPage, int limit) {
		Connection conn = getConnection();
		ArrayList<Product_Hoogi> list = Pdao.hoogiSearchTCmix(conn, word, currentPage, limit);
		close(conn);
		return list;
	}

	public ArrayList<Product_Hoogi> hoogiSearchWriter(String word, int currentPage, int limit) {
		Connection conn = getConnection();
		ArrayList<Product_Hoogi> list = Pdao.hoogiSearchWriter(conn, word, currentPage, limit);
		close(conn);
		return list;
	}

	public int deleteComment(int hoogiNo) {
		Connection conn = getConnection();
		int result = Pdao.deleteComment(conn, hoogiNo);
		if(result > 0)
			commit(conn);
		else
			rollback(conn);
